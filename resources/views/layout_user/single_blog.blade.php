@include('layout_user.menu')
<!-- BREADCRUMBS SETCTION START -->
<div class="breadcrumbs-section plr-200 mb-80">
    <div class="breadcrumbs overlay-bg">
        <div class="container">
            <div class="row">
                <div class="col-xs-12">
                    <div class="breadcrumbs-inner">
                        <h1 class="breadcrumbs-title">Chi tiết bài viết</h1>
                        <ul class="breadcrumb-list">
                            <li><a href="/">Trang chủ</a></li>
                            <li>Chi tiết bài viết</li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- BREADCRUMBS SETCTION END -->

<!-- Start page content -->
<section id="page-content" class="page-wrapper">

    <!-- PHẦN BLOG BẮT ĐẦU -->
    <div class="blog-section mb-50">
        <div class="container">
            <div class="row">
                <div class="col-md-9 col-xs-12">
                    <div class="blog-details-area">
                        <!-- ảnh chi tiết blog -->
                        <div class="blog-details-photo bg-img-1 p-20 mb-30">
                            <img src="/img/blog/10.jpg" alt="">
                            <div class="today-date bg-img-1">
                                <span class="meta-date">30</span>
                                <span class="meta-month">Tháng Sáu</span>
                            </div>
                        </div>
                        <!-- thích và chia sẻ blog -->
                        <ul class="blog-like-share mb-20">
                            <li>
                                <a href="#"><i class="zmdi zmdi-favorite"></i>89 Thích</a>
                            </li>
                            <li>
                                <a href="#"><i class="zmdi zmdi-comments"></i>59 Bình luận</a>
                            </li>
                            <li>
                                <a href="#"><i class="zmdi zmdi-share"></i>29 Chia sẻ</a>
                            </li>
                        </ul>
                        <!-- tiêu đề chi tiết blog -->
                        <h3 class="blog-details-title mb-30">{{ $post->title }}</h3>
                        <!-- mô tả blog -->
                        <div class="blog-description mb-60">
                            <p>{{ $post->content }}</p>
                        </div>
                        <!-- chia sẻ và thẻ blog -->
                        <div class="blog-share-tags box-shadow clearfix mb-60">
                            <div class="blog-share f-left">
                                <p class="share-tags-title f-left">chia sẻ</p>
                                <ul class="footer-social f-left">
                                    <li>
                                        <a class="facebook" href="" title="Facebook"><i
                                                class="zmdi zmdi-facebook"></i></a>
                                    </li>
                                    <li>
                                        <a class="google-plus" href="" title="Google Plus"><i
                                                class="zmdi zmdi-google-plus"></i></a>
                                    </li>
                                    <li>
                                        <a class="twitter" href="" title="Twitter"><i class="zmdi zmdi-twitter"></i></a>
                                    </li>
                                    <li>
                                        <a class="rss" href="" title="RSS"><i class="zmdi zmdi-rss"></i></a>
                                    </li>
                                </ul>
                            </div>
                            <div class="blog-tags f-right">
                                <p class="share-tags-title f-left">Thẻ</p>
                                <ul class="blog-tags-list f-left">
                                    <li><a href="#">Di động</a></li>
                                    <li><a href="#">IOS</a></li>
                                    <li><a href="#">Windows</a></li>
                                    <li><a href="#">Máy tính bảng</a></li>
                                </ul>
                            </div>
                        </div>
                        <!-- tác giả bài viết -->
                        <div class="media author-post box-shadow mb-60">
                            <div class="media-left pr-20">
                                <a href="#">
                                    <img class="media-object" src="/img/author/1.jpg" alt="#">
                                </a>
                            </div>
                            <div class="media-body">
                                <h6 class="media-heading">
                                    <a href="#">Subash Chandra Das</a>
                                </h6>
                                <p class="mb-0">Không ai từ chối, không thích, hoặc tránh né niềm vui bản thân, bởi vì
                                    đó là niềm vui, mà bởi vì những người không biết cách theo đuổi niềm vui một cách
                                    hợp lý sẽ gặp phải những hậu quả cực kỳ đau đớn.</p>
                            </div>
                        </div>
                        <!-- bình luận về bài viết này -->
                        <div class="post-comments mb-60">
                            <h4 class="blog-section-title border-left mb-30">bình luận về sản phẩm này</h4>
                            <!-- bình luận đơn -->
                            <div class="media mt-30">
                                <div class="media-left pr-30">
                                    <a href="#"><img class="media-object" src="/img/author/2.jpg" alt="#"></a>
                                </div>
                                <div class="media-body">
                                    <div class="clearfix">
                                        <div class="name-commenter f-left">
                                            <h6 class="media-heading"><a href="#">Gerald Barnes</a></h6>
                                            <p class="mb-10">27 Tháng Sáu, 2016 lúc 2:30 chiều</p>
                                        </div>
                                        <ul class="reply-delate f-right">
                                            <li><a href="#">Phản hồi</a></li>
                                            <li>/</li>
                                            <li><a href="#">Xóa</a></li>
                                        </ul>
                                    </div>
                                    <p class="mb-0">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer
                                        accumsan egestas elese ifend. Phasellus a felis atestese bibendum feugiat ut
                                        eget eni Praesent messages in con sectetur posuere dolor non.</p>
                                </div>
                            </div>
                            <!-- bình luận đơn -->
                            <div class="media mt-30">
                                <div class="media-left pr-30">
                                    <a href="#"><img class="media-object" src="/img/author/3.jpg" alt="#"></a>
                                </div>
                                <div class="media-body">
                                    <div class="clearfix">
                                        <div class="name-commenter f-left">
                                            <h6 class="media-heading"><a href="#">Gerald Barnes</a></h6>
                                            <p class="mb-10">27 Tháng Sáu, 2016 lúc 2:30 chiều</p>
                                        </div>
                                        <ul class="reply-delate f-right">
                                            <li><a href="#">Phản hồi</a></li>
                                            <li>/</li>
                                            <li><a href="#">Xóa</a></li>
                                        </ul>
                                    </div>
                                    <p class="mb-0">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer
                                        accumsan egestas elese ifend. Phasellus a felis atestese bibendum feugiat ut
                                        eget eni Praesent messages in con sectetur posuere dolor non.</p>
                                </div>
                            </div>
                        </div>
                        <!-- để lại bình luận của bạn -->
                        <div class="leave-comment">
                            <h4 class="blog-section-title border-left mb-30">để lại bình luận của bạn</h4>
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="name">Tên của bạn <span>*</span></label>
                                        <input type="text" class="form-control" id="name"
                                            placeholder="Nhập tên của bạn...">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="email">Địa chỉ email <span>*</span></label>
                                        <input type="email" class="form-control" id="email"
                                            placeholder="Nhập địa chỉ email của bạn...">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="message">Tin nhắn <span>*</span></label>
                                <textarea class="form-control" id="message" rows="5"
                                    placeholder="Nhập tin nhắn của bạn..."></textarea>
                            </div>
                            <button class="btn btn-primary">Gửi bình luận</button>
                        </div>
                    </div>
                </div>
                <!-- Phần sidebar -->
                <div class="col-md-3 col-xs-12">
                    <div class="blog-sidebar">
                        <!-- Tìm kiếm blog -->
                        <div class="widget-search mb-50">
                            <h5 class="widget-title mb-30">Tìm kiếm</h5>
                            <form action="#">
                                <input type="text" placeholder="Tìm kiếm blog...">
                                <button type="submit"><i class="zmdi zmdi-search"></i></button>
                            </form>
                        </div>
                        <!-- danh mục blog -->
                        <div class="widget-categories mb-50">
                            <h5 class="widget-title mb-30">Danh mục blog</h5>
                            <ul>
                                <li><a href="#">Tin tức công nghệ</a></li>
                                <li><a href="#">Mẹo và thủ thuật</a></li>
                                <li><a href="#">Phát triển ứng dụng</a></li>
                                <li><a href="#">Tư vấn kỹ thuật</a></li>
                            </ul>
                        </div>
                        <!-- bài viết nổi bật -->
                        <div class="widget-popular-post mb-50">
                            <h5 class="widget-title mb-30">Bài viết nổi bật</h5>
                            <div class="media mb-30">
                                <div class="media-left">
                                    <a href="#">
                                        <img class="media-object" src="/img/blog/1.jpg" alt=""
                                            style="width:120px; height:auto;">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <h6 class="media-heading"><a href="#">Bài viết nổi bật 1</a></h6>
                                    <span class="met-date">30 Tháng Sáu</span>
                                </div>
                            </div>
                            <div class="media mb-30">
                                <div class="media-left">
                                    <a href="#">
                                        <img class="media-object" src="/img/blog/2.jpg" alt=""
                                            style="width:120px; height:auto;">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <h6 class="media-heading"><a href="#">Bài viết nổi bật 2</a></h6>
                                    <span class="met-date">30 Tháng Sáu</span>
                                </div>
                            </div>
                            <div class="media mb-30">
                                <div class="media-left">
                                    <a href="#">
                                        <img class="media-object" src="/img/blog/3.jpg" alt=""
                                            style="width:120px; height:auto;">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <h6 class="media-heading"><a href="#">Bài viết nổi bật 3</a></h6>
                                    <span class="met-date">30 Tháng Sáu</span>
                                </div>
                            </div>
                        </div>
                        <!-- thư mục lưu trữ -->
                        <div class="widget-archive mb-50">
                            <h5 class="widget-title mb-30">Lưu trữ</h5>
                            <ul>
                                <li><a href="#">Tháng Sáu 2022</a></li>
                                <li><a href="#">Tháng Năm 2022</a></li>
                                <li><a href="#">Tháng Tư 2022</a></li>
                                <li><a href="#">Tháng Ba 2022</a></li>
                                <li><a href="#">Tháng Hai 2022</a></li>
                                <li><a href="#">Tháng Giêng 2022</a></li>
                            </ul>
                        </div>
                        <!-- thẻ blog -->
                        <div class="widget-tags mb-50">
                            <h5 class="widget-title mb-30">Thẻ</h5>
                            <ul class="tag-list">
                                <li><a href="#">Công nghệ</a></li>
                                <li><a href="#">Thiết bị</a></li>
                                <li><a href="#">Mẹo</a></li>
                                <li><a href="#">Hướng dẫn</a></li>
                            </ul>
                        </div>
                        <!-- xã hội -->
                        <div class="widget-social mb-50">
                            <h5 class="widget-title mb-30">Kết nối với chúng tôi</h5>
                            <ul class="footer-social">
                                <li>
                                    <a class="facebook" href="" title="Facebook"><i class="zmdi zmdi-facebook"></i></a>
                                </li>
                                <li>
                                    <a class="google-plus" href="" title="Google Plus"><i
                                            class="zmdi zmdi-google-plus"></i></a>
                                </li>
                                <li>
                                    <a class="twitter" href="" title="Twitter"><i class="zmdi zmdi-twitter"></i></a>
                                </li>
                                <li>
                                    <a class="rss" href="" title="RSS"><i class="zmdi zmdi-rss"></i></a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- PHẦN BLOG KẾT THÚC -->
</section>
<!-- End page content -->
@include('layout_user.footer')