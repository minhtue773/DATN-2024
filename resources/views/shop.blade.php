    ...
    <div class="product-item">
        ...
        <div class="product-info">
            ...
            <ul class="action-button">
                @auth
                <li>
                    <a href="#" class="wishlist-toggle" data-product-id="{{ $product->id }}" title="Wishlist">
                        <i class="zmdi zmdi-favorite {{ Auth::user()->favorites()->where('product_id', $product->id)->exists() ? 'favorited' : '' }}"></i>
                    </a>
                </li>
                @endauth
                <li>
...
<li class="{{ request('category_id') === null ? 'open' : 'closed' }}">
    <a href="{{ route('products.index', ['category_id' => null, 'sort_by' => request('sort_by')]) }}" style="font-size: 1.2em; color: #000000; text-decoration: none;">Tất cả</a>
</li>
@foreach ($categories as $category)
<li class="{{ request('category_id') == $category->id ? 'open' : 'closed' }}">
    <a
        href="{{ route('products.index', ['category_id' => $category->id, 'sort_by' => request('sort_by')]) }}" style="font-size: 1.2em; color: #000000; text-decoration: none; display: flex; justify-content: space-between; align-items: center;">
        {{ $category->name }}
        <span class="product-count" style="margin-left: auto; color: #888;">({{ $category->products->count() }})</span>
    </a>
</li>
@endforeach
@endsection